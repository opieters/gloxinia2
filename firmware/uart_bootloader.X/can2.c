...
bool parse_from_can2_buffer(bootloader_message_t *m, uint16_t *raw_data)
{
    uint16_t i, identifier;
    ...
    m->address_low = extended_identifier & 0xFFU;
    m->unlock = (extended_identifier >> 16) == CAN2_UNLOCK_SEQUENCE ? true : false;
    ...
    return (!remote_frame) && (extended_frame) && (identifier == BOOTLOADER_ADDRESS);
}


can_status_t can2_send_message(bootloader_message_t *message, uint8_t can_channel)
{
    uint16_t i;
    ...
    identifier |= (message->command << 8);

    identifier |= message->address_low;

    ecan_message_buffer[can_channel][0] = ((BOOTLOADER_ADDRESS & 0x7FFU) << 2) | (false == 0 ? 0b00 : 0b11) | (false == 0 ? 0b00 : 0b10);
...
can_status_t can2_send_message_any_ch(bootloader_message_t *m)
{
    static uint16_t i = 0;
