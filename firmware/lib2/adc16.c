...
unsigned int adc_rx_buffer_b[ADC_MAX_BUFFER_LENGTH] __attribute__((space(dma), eds, address(0xD830)));

void (*rx_callback)(void) = adc_callback_dummy;

void adc_callback_dummy(void){
...
void adc_update(adc_config_t* config){
    spi_message_t m;
...
void adc_init(adc_config_t* config){
    ...
    DMA4CNT = ADC_TX_BUFFER_LENGTH-1;
    ...
    DMA5CNT = config->adc_buffer_size-1;
    ...
    for(i = 0; i < ADC_MAX_BUFFER_LENGTH; i++){        
        adc_rx_buffer_a[i] = 0;
        adc_rx_buffer_b[i] = 0;
    }
    DSRPAG = __builtin_edspage(adc_tx_buffer);
    for(i = 0; i < ADC_TX_BUFFER_LENGTH; i++){
    ...
    for(i = 0; i < N_ADC_CHANNELS; i++){
        ...
        read_data[0] = adc_read_channel(config);
...
void adc_init_fast(adc_config_t* config){
    spi_message_t m;
    uint16_t write_data[2];
    uint16_t read_data[2];
    read_data[0] = 0x0;
...
uint16_t adc_read_channel(adc_config_t* config){
...
int16_t adc_run_calibration(adc_config_t* config, 
    ...
    for(n = 0; n < ADC_N_CALIBRATION_SAMPLES; n++){
        sample_data += adc_read_channel(config);
    }
...
void adc_run_max_var(adc_config_t* config,  uint16_t* const max_value, 
    ...
    *min_value = 0;
    ...
    // read all data
    for(n = 0; n < ADC_N_CALIBRATION_SAMPLES; n++){
        sample_value = adc_read_channel(config);
