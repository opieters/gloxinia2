...
uint8_t uart_dma_rx_message_data[17] __attribute__( (aligned(32)) );;
...
void process_uart_tx_queue(void)
{
    ...
    else if ((uart_tx_ongoing == 0) && (n_uart_tx_messages2 > 0))
    {
        ...
        uart_dma_tx_message_data[4] = 0;
        uart_dma_tx_message_data[5] = M_MSG_TEXT;
        uart_dma_tx_message_data[6] = (NO_INTERFACE_ID << 4) | NO_SENSOR_ID;
...
{   
    ...
    if((uart_dma_rx_message_data[0] == UART_CMD_START) && (uart_dma_rx_message_data[ARRAY_LENGTH(uart_dma_rx_message_data)-1] == UART_CMD_STOP)){
        ...
        message_init(m, 
                ...
                uart_dma_rx_message_data[6] >> 4,
                uart_dma_rx_message_data[6] & 0xf,
                &uart_dma_rx_message_data[8],
