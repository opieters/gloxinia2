...
fractional i2c_raw_channel_data[SYLVATICA_N_CHANNELS];

sylvatica_status_t sylvatica_status = SYLVATICA_STATUS_INIT;

size_t n_coeffs_written = 0;
...
void process_filter_block0(void){
    ...
    for(i = 0; i < SYLVATICA_N_CHANNELS; i++){   
        ...
        if(sample_counter == 10000){
            i2c_raw_channel_data[i] = sample_buffer[0];
        }
...
void process_filter_block3(void){
    ...
    n_coeffs_written = MIN(n_coeffs_written+SYLVATICA_BLOCK3_OUTPUT_SIZE,filters_3[0].numCoeffs);
...
void i2c_mw_sr_cb_sylvatica(i2c_message_t* m){
    ...
    switch(i2c_write_reg){
        ...
        case SYLVATICA_REG_ADC:
            ...
            sylvatica_i2c_read_config(i2c_write_reg);
            break;
        case SYLVATICA_REG_CONFIG_CH0:
        case SYLVATICA_REG_CONFIG_CH1:
        case SYLVATICA_REG_CONFIG_CH2:
        case SYLVATICA_REG_CONFIG_CH3:
        case SYLVATICA_REG_CONFIG_CH4:
        case SYLVATICA_REG_CONFIG_CH5:
        case SYLVATICA_REG_CONFIG_CH6:
        case SYLVATICA_REG_CONFIG_CH7:
            if(m->data_length >= 2){ 
                sylvatica_i2c_channel_config(i2c_write_reg - SYLVATICA_REG_CONFIG_CH0, &m->data[1]);
            }
            sylvatica_i2c_read_config(i2c_write_reg);
            break;
        case SYLVATICA_REG_DATA:
            sylvatica_i2c_read_copy_channel_data();
            break;
        case SYLVATICA_REG_DATA_RAW:
            sylvatica_i2c_read_copy_raw_data();
            break;
        default:
            break;
    }
...
void sylvatica_i2c_read_copy_channel_data(){  
    ...
    i2c_mr_message_data[0] = SYLVATICA_N_CHANNELS*2;
    
    for(i = 0; i < SYLVATICA_N_CHANNELS; i++){
        // copy data
        i2c_mr_message_data[1+2*i]   = (uint8_t) ((output_buffer_a[i] >> 8) & 0xff);
        i2c_mr_message_data[1+2*i+1] = (uint8_t) (output_buffer_a[i] & 0xff); 
    }    

    i2c_init_read_message(
        &i2c_mr_message,
        i2c_mr_message_data,
        1+SYLVATICA_N_CHANNELS*2);
...
void sylvatica_i2c_read_copy_raw_data(){
    ...
    for(i = 0; i < SYLVATICA_N_CHANNELS; i++){
        // copy data
        i2c_mr_message_data[1+2*i]   = (uint8_t) ((i2c_raw_channel_data[i] >> 8) & 0xff);
        i2c_mr_message_data[1+2*i+1] = (uint8_t) (i2c_raw_channel_data[i] & 0xff);  
    }    
