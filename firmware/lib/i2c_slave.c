...
extern volatile i2c_slave_state_t i2c2_slave_state;
    ...
    switch(i2c1_slave_state){
        ...
        case I2C_SLAVE_STATE_WRITE:
            if(i2c_slave_mr_sw_message->status == I2C_MESSAGE_READ_READY){
                ...
                if(n_transfers < i2c_slave_mr_sw_message->write_length){
                    // read valid message data
                    I2C1TRN = i2c_slave_mr_sw_message->write_data[n_transfers];
                    n_transfers++;
                    
                    //sprintf(print_buffer, "I2C write: %02x", i2c_slave_mr_sw_message->data[n_transfers-1]);
                    ...
                    //uart_simple_print("I2C write overflow.");
                ...
                //uart_simple_print("I2C m not init.");
        ...
        case I2C_SLAVE_STATE_READ:
            if(n_transfers < I2C_W_MESSAGE_BUFFER_LENGTH){
                i2c_slave_mw_sr_message.write_data[n_transfers] = I2C1RCV;
                n_transfers++;
                i2c_slave_mw_sr_message.write_length = n_transfers;
                //I2C1CONbits.ACKDT = 1;
            } else {
    ...
    switch(i2c1_slave_state){
        ...
        case I2C_SLAVE_STATE_ADDRESS:
            if(I2C1STATbits.R_W == 1){
                // received the write address -> master reads data
                if(i2c_slave_mr_sw_message != NULL){
                    ...
                    if(i2c_slave_mr_sw_message->status == I2C_MESSAGE_READ_READY) {
                        if(n_transfers < i2c_slave_mr_sw_message->write_length){
                            ...
                            //uart_simple_print("I2C write overflow 2");
                        }
                    } else {
                        i2c_slave_mr_sw_message->error = I2C_MSG_NOT_INITIALISED;
                        ...
                        //uart_simple_print("I2C write no message.");
                    }
                }
                else {
