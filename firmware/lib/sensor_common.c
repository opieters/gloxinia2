...
void sensor_send_general_error(sensor_elog_t* elog, uint8_t status, uint8_t error){
    if(controller_address == 0) {
        if((elog->uart_message.status == UART_MSG_SENT) || (elog->uart_message.status == UART_MSG_INIT_DONE)){
            elog->uart_message.data[2] = status;
            elog->uart_message.data[3] = error;
            uart_reset_message(&elog->uart_message);
            uart_queue_message(&elog->uart_message);
        }
    } else {
        elog->can_message.data[2] = status;
        elog->can_message.data[3] = error;
